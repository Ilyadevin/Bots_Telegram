from bots.bot_meeting_app.meeting_bot import *


@bot_meeting.message_handler(commands=['help'])
def help_user(message):
    try:
        time.sleep(2)
        bot_meeting.send_message(message.chat.id, 'Я - бот который поможет вам найти пару для приятного общения.\n'
...
@bot_meeting.message_handler(content_types=['text'])
def setting_user(message):
    try:
        if message.text.lower() == 'настройки' or \
            ...
            bot_meeting.send_message(message.from_user.id, 'Введите логин и пароль в формате "password login"')
